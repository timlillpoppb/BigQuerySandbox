...
c1.metric("Avg LTV:CAC Ratio",    f"{overall_ltv_cac:.2f}×",
          help="Predicted 12m LTV / First Order Value. >3× is healthy.")
c2.metric("Avg Payback Period",   f"{overall_payback:.1f} months",
...
with col2:
    ...
    df_trend = (
        df
        .groupby(["cohort_month", "acquisition_channel"], as_index=False)
        .agg(avg_predicted_12m_ltv = ("avg_predicted_12m_ltv", "mean"))
    )
