...
with col_f2:
    max_period = st.slider("Max period (months)", min_value=3, max_value=24, value=12, step=1)

df_seg = df if seg == "All" else df[df.customer_segment == seg]

df_agg = (
    df_seg
    .groupby(["cohort", "period_number"], as_index=False)
    .agg(
...
pivot = (
    df_agg
    .pivot(index="cohort", columns="period_number", values="retention_rate")
    .sort_index(ascending=False)   # newest cohort on top
)
...
with col2:
    ...
    df_m0 = (
        df_agg[df_agg.period_number == 0][["cohort", "cohort_size"]]
        .sort_values("cohort")
    )
    fig_sizes = go.Figure(go.Bar(
        x            = df_m0.cohort,
        y            = df_m0.cohort_size,
        marker_color = "#4F8BF9",
        name         = "Cohort Size",
