...
selected_month = st.selectbox("Select month for waterfall", months, index=len(months) - 1)
...
with col2:
    ...
    pie_vals   = [abs(row.new_mrr), abs(row.expansion_mrr), abs(row_full.retained_mrr),
                  abs(row.contraction_mrr), abs(row.churned_mrr)]
