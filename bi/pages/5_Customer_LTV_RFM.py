...
st.caption("Lifetime value distribution, predicted LTV, and RFM segmentation — powered by `rpt_customer_ltv`")
...
with col2:
    ...
    st.caption("Points above the diagonal are over-predicted; below are under-predicted.")
    df_scatter = (
        df[(df.observed_ltv > 0) & (df.predicted_12m_ltv > 0)]
...
df_rfm = (
    df
    .groupby(
        ["rfm_recency_score", "rfm_frequency_score", "rfm_monetary_score", "customer_segment"],
        as_index=False,
    )
    .agg(
        count   = ("user_id",       "count"),
...
with col3:
    ...
    df_ch = (
        df
        .groupby(["acquisition_channel", "customer_segment"], as_index=False)
        .agg(avg_predicted = ("predicted_12m_ltv", "mean"))
    )
