...
st.caption("Revenue decomposed by segment, channel, category, country — powered by `rpt_revenue_by_segment`")
...
df_seg_time = (
    df_f
    .groupby(["billing_month", "customer_segment"], as_index=False)
...
with col1:
    ...
    df_cat = (
        ...
        .assign(gm_pct = lambda x: (x.total_gross_margin / x.total_revenue * 100).round(1))
...
df_sub = (
    df_f
    .groupby("billing_month", as_index=False)
    .agg(
...
df_ch_hm = (
    df_f
    .groupby(["billing_month", "acquisition_channel"], as_index=False)
    .agg(total_revenue = ("total_revenue", "sum"))
)
pivot_ch = (
    df_ch_hm
    .pivot(index="acquisition_channel", columns="billing_month", values="total_revenue")
    .fillna(0)
)
