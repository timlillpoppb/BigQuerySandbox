...
st.caption("Single source of truth for C-suite metrics — powered by `rpt_subscription_health`")
...
c1.metric("Total MRR",             fmt_currency(latest.total_mrr),
          delta_pct(latest.total_mrr, prev.total_mrr))
c2.metric("ARR",                   fmt_currency(latest.arr))
c3.metric("Active Subscribers",    f"{int(latest.active_subscribers):,}",
          delta_pct(latest.active_subscribers, prev.active_subscribers))
c4.metric("Subscriber Churn",      fmt_pct(latest.subscriber_churn_rate),
          delta_pct(latest.subscriber_churn_rate, prev.subscriber_churn_rate),
          delta_color="inverse")
c5.metric("Net Revenue Retention", fmt_pct(latest.net_revenue_retention_rate),
          delta_pct(latest.net_revenue_retention_rate, prev.net_revenue_retention_rate))
c6.metric("ARPU",                  fmt_currency(latest.arpu),
          delta_pct(latest.arpu, prev.arpu))
...
with col_r:
    ...
    colors_growth = ["#4CAF50" if v >= 0 else "#F44336"
                     for v in df.mrr_growth_rate.fillna(0)]
